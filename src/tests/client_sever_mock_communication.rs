...
pub(super) fn unlock_vault_for_organization(server: &mut LocalServer, organization_name: &str,
...
pub(super) fn upload(document: &Document, token: &Token, client: &ClientEncryptorDecryptor, server: &LocalServer) -> Option<()> {
...
pub(super) fn share(document_id: &DocumentID, other_organization_name: &str,
                    token: &Token, client: &ClientEncryptorDecryptor, server: &LocalServer) -> Option<()> {
...
pub(super) fn download_from_document_id(document_id: &DocumentID, token: &Token, client: &ClientEncryptorDecryptor, server: &LocalServer) -> Document {
...
pub(super) fn download_from_document_name(document_name: &str, token: &Token, client: &ClientEncryptorDecryptor, server: &LocalServer) -> Document {
...
pub(super) fn update(document_id: &DocumentID, new_document: &Document, token: &Token, client: &ClientEncryptorDecryptor, server: &LocalServer)
                     -> Option<()> {
...
pub(super) fn get_id_of_document_by_name(document_name: &str, token: &Token, client: &ClientEncryptorDecryptor, server: &LocalServer) -> Option<DocumentID> {
    let document_list = server.list_documents(token).unwrap();
